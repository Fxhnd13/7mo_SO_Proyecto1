...
static void * actualizacion(void* arg){
    ...
    printf("Actualizar interfaz xd");
    sem_post(&semaforo);
}

static void crear(int tallo, int rama, int hoja){
    Tallo tallo = Tallo(tallo,0,0);
    agregarSemaforo(&tallo);
    int p = fork();
    if(p < 0){
        //error
    }else if(p == 0){
        tallo.crearRamas(rama,hoja);
    }
    pidHijos.push_back(p);
    agregarTallo(tallo);
}
